...
export default function TaxRateEdit({ tax_rate }: Props) {
  const { data, setData, put, processing, errors } = useForm({
    name: tax_rate.name,
    rate: tax_rate.rate.toString(),
    is_compound: tax_rate.is_compound,
    is_active: tax_rate.is_active,
  });

  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    put(route('tax_rate.update', tax_rate.id));
  };

  return (
    <AppLayout title={`Edit Tax Rate: ${tax_rate.name}`}>
      <Head title={`Edit Tax Rate: ${tax_rate.name}`} />
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-5">
          <h3 className="text-lg leading-6 font-medium text-gray-900">Edit Tax Rate</h3>
