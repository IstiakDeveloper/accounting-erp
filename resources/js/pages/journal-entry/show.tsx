...
export default function JournalEntryShow({ journal_entry }: Props) {
    ...
    const debit = parseFloat(journal_entry.debit_amount || 0);
    const credit = parseFloat(journal_entry.credit_amount || 0);

    const entryType = debit > 0 ? 'debit' : 'credit';
    const entryAmount = entryType === 'debit' ? debit : credit;
                        ...
                        <div className="sm:col-span-2">
                            ...
                            <dd className="mt-1 flex items-center text-sm text-slate-900">
                                ...
                                <div>
                                    ...
                                    <span className="text-slate-500 ml-2">({journal_entry.ledger_account.code})</span>
                                </div>
                            </dd>
                        </div>
                                    ...
                                    <table className="min-w-full divide-y divide-slate-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-slate-200">
                                            ...
                                            <tr className="bg-slate-50 font-medium">
                                                ...
                                                <td className="px-6 py-3 whitespace-nowrap text-sm text-slate-900 text-right">
                                                    {formatCurrency(
                                                        journal_entry.voucher.voucher_items.reduce((sum, item) => {
                                                            // ensure each debit_amount is a number
                                                            return sum + (parseFloat(item.debit_amount) || 0);
                                                        }, 0)
                                                    )}
                                                </td>
                                                ...
                                                <td className="px-6 py-3 whitespace-nowrap text-sm text-slate-900 text-right">
                                                    {formatCurrency(
                                                        journal_entry.voucher.voucher_items.reduce((sum, item) => {
                                                            // Ensure that credit_amount is a valid number
                                                            return sum + (parseFloat(item.credit_amount) || 0);
                                                        }, 0)
                                                    )}
                                                </td>
