...
export default function BudgetShow({ budget, budget_vs_actual, monthly_budget }: Props) {
  // Calculate totals
  const totalBudget = Object.values(budget_vs_actual.budget).reduce((sum, amount) => sum + amount, 0);
  const totalActual = Object.values(budget_vs_actual.actual).reduce((sum, amount) => sum + amount, 0);
  ...
  const incomeItems = budget.budget_items.filter(item => item.ledger_account.account_group.nature === 'income');
  const expenseItems = budget.budget_items.filter(item => item.ledger_account.account_group.nature === 'expense');
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className={`text-lg font-semibold flex items-center ${totalVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      ...
                      {totalVariance >= 0 ? '+' : ''}{totalVariancePercentage.toFixed(2)}%
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.entries(monthly_budget).map(([month, amount]) => (
                  <div key={month}>
              ...
              <div className="bg-white shadow overflow-hidden sm:rounded-md">
                <ul className="divide-y divide-slate-200">
                  {incomeItems.map((item) => {
                    ...
                    const budgetAmount = item.annual_amount;
                    const actualAmount = budget_vs_actual.actual[accountId] || 0;
                    const variance = budget_vs_actual.variance[accountId] || 0;
                    const variancePercentage = budget_vs_actual.variance_percentage[accountId] || 0;
